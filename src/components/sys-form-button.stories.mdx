...
SysFormButton is a basic wrapper component for the native HTML `button` and for
Vue Router `router-link` (or `nuxt-link` if you are using Nuxt).
...
There are three different colors for `SysFormButton`. Each one works the exact
same way, but changes the appearance.
